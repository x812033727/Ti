...
export function addRosterMember(r) {
  ...
  el.innerHTML = `
      <div class="av">${r.avatar || '<svg class="icon" aria-hidden="true"><use href="#i-plus"/></svg>'}</div>
...
export function addSystem(text, cls, iconName) {
  const el = document.createElement("div");
  el.className = "sys" + (cls ? " " + cls : "");
  if (iconName) el.appendChild(icon(iconName, "icon sys-ic"));
  // 訊息本體一律走 textContent（XSS 守護：qa_test_render_agenda_criteria）
...
export function addResult(passed, detail, log) {
  ...
  el.appendChild(icon(passed ? "check" : "x", "icon sys-ic"));
  const span = document.createElement("span");
...
export function addHuman(text) {
  ...
  el.innerHTML = `
    <div class="av"><svg class="icon" aria-hidden="true"><use href="#i-user"/></svg></div>
...
export function addDemo(p) {
  ...
  el.innerHTML = `<div class="demohead"><svg class="icon" aria-hidden="true"><use href="#i-play"/></svg>${p.passed ? "Demo 執行成功" : "Demo 執行失敗"} <code></code></div>`;
  ...
  if (p.retried_cmd) {
    ...
    note.textContent =
      "原指令帶無效參數已消毒重試：" + p.command + "（exit " + p.first_exit + "）";
    el.appendChild(note);
  }
  const pre = document.createElement("pre");
...
export const CI_LABELS = {
  pass: ["CI 通過", "pass", "check"],
  none: ["無 CI 檢查，直接合併", "pass", "check"],
  fail: ["CI 未通過", "fail", "x"],
  error: ["CI 等待逾時/出錯，保留 PR 待人工", "fail", "alert"],
  merged: ["已自動合併（squash + 刪分支）", "pass", "merge"],
  merge_failed: ["合併失敗", "fail", "x"],
  giveup: ["CI 連續失敗達上限，保留 PR 待人工", "fail", "stop"],
};

export function addCI(p) {
  const [label, cls, ic] = CI_LABELS[p.state] || [p.state || "CI", "fail", "refresh"];
  const round = p.attempt && p.rounds ? `（第 ${p.attempt}/${p.rounds} 輪）` : "";
  const el = document.createElement("div");
  el.className = "ci result " + cls;
  el.appendChild(icon(ic, "icon sys-ic"));
  const span = document.createElement("span");
...
export function handleEvent(ev) {
  ...
  switch (ev.type) {
    case "session_started":
      ...
      addSystem("工作室開工：" + (p.requirement || ""), "", "wrench");
      if (p.repo_url) addSystem("既有專案：" + p.repo_url, "", "box");
      break;
    case "phase_change":
      setPhase(p.phase);
    ...
    case "dispatch_decision": {
      ...
      addSystem(
        `${modeTag} 任務 #${p.task_id ?? "?"} → ${p.role || ""}@${target}`.trim() +
          (p.reason ? `（${p.reason}）` : ""),
        "",
        "route",
      );
    ...
    case "clarify_request": {
      ...
      addSystem("PM 想先跟你確認需求（在下方插話框回答，一則訊息回答全部即可）：", "", "help");
      (p.questions || []).forEach((q, i) => {
        addSystem(`${i + 1}. ${q.q}` + (q.assumption ? `（未回覆時假設：${q.assumption}）` : ""));
      });
      if (p.timeout_s) addSystem(`${Math.round(p.timeout_s)} 秒內未回覆，將按 PM 的預設假設繼續。`, "", "clock");
    ...
    case "agenda_plan": {
      ...
      addSystem(`議程拆解：${items.length} 個子題`, "", "news");
      items.forEach((a, i) => {
      ...
      (p.corrections || []).forEach((c) => {
        addSystem(`分派修正：子題 ${c.index + 1} 的「負責: ${c.given || "（缺漏）"}」→ ${c.assigned}`, "", "refresh");
      });
      break;
    }
    case "conclusion": {
      // 結論彙整快照：一場討論收斂後產出 CONCLUSION.md，渲染四段摘要（重播時亦經此）。
      addSystem("結論彙整：已產出 CONCLUSION.md", "", "doc");
      const s = p.summary || {};
      const sections = [
        ["共識", s.consensus],
        ["分歧", s.disagreements],
    ...
    case "critic_review":
      if (p.passed) {
        addSystem("異議檢查放行（" + (p.gate || "") + " 視角）", "", "search");
      } else {
        addSystem("異議檢查退回（" + (p.gate || "") + " 視角）：" + (p.text || ""), "", "stop");
      }
      break;
    case "huddle":
      if (p.limitation) {
        addSystem("已知限制：任務「" + (p.title || "") + "」huddle 後仍未通過。", "", "alert");
      } else {
        const who = (p.participants || []).join("、");
        addSystem("卡關討論：任務「" + (p.title || "") + "」召集 " + who + " 找替代方案。", "", "users");
    ...
    case "task_result": {
      ...
      addSystem(`任務 #${p.task_id ?? "?"} 結果 ▸ ${who}${bits.length ? `（${bits.join("・")}）` : ""}`, "", "chart");
      break;
    }
    case "done":
      ...
      if (state.improveMode && !p.improve) {
        addSystem(
          p.completed ? "本輪改良完成，繼續下一輪…" : "本輪未達標，迴圈將評估是否續跑…",
          "",
          p.completed ? "check" : "alert",
        );
        refreshFiles();
        break;
      }
      if (p.improve) {
        const s = p.improve;
        setPhase(p.stopped ? "已停止" : "持續改良結束");
        addSystem(`持續改良結束：共 ${s.cycles} 輪（完成 ${s.done}、未達標 ${s.failed}）。`, "", "refresh");
      } else {
        setPhase(p.stopped ? "已停止" : (p.completed ? "已完成" : "結束（未完全達標）"));
        addSystem(
          p.stopped ? "已依指示停止。" : (p.completed ? "專案完成！" : "專案結束，仍有未達標項目。"),
          "",
          p.stopped ? "stop" : (p.completed ? "stamp" : "flag"),
        );
      }
      refreshFiles();
      ...
      break;
    case "error":
      addSystem("錯誤：" + (p.message || "未知錯誤"), "", "alert");
...
export function addPublishButton(sid) {
  ...
  btnLabel.textContent = state.mergeEnabled ? "發佈並合併到 GitHub" : "發佈成果到 GitHub";
  btn.appendChild(btnLabel);
  btn.onclick = async () => {
    btn.disabled = true; btnLabel.textContent = state.mergeEnabled ? "發佈並合併中…" : "發佈中…";
    try {
...
const ICON_HTML = (name) => `<svg class="icon sys-ic" aria-hidden="true"><use href="#i-${name}"/></svg>`;

export function renderPublish(p) {
  const el = document.createElement("div");
  el.className = "publish " + (p.ok ? "ok" : "fail");
  let html = ICON_HTML(p.ok ? "send" : "alert") + (p.detail || "");
