...
function scheduleReconnect() {
  ...
  reconnectTimer = setTimeout(() => {
    ...
    sock.onopen = () => sock.send(JSON.stringify({ attach: sessionId, cursor: eventCount }));
    bindSocket(sock);
    ws = sock;
  }, delay);
}
...
function bindSocket(sock) {
  sock.onmessage = (e) => {
    ...
    if (ev.type === "error" && (ev.payload || {}).code === "attach_unavailable") {
      ...
      stopReconnect();
      if (sessionId) replaySession(sessionId); else setRunning(false);
      return;
    }
  ...
  // 正常收尾/重播/持續改良模式維持原行為。
  sock.onclose = () => {
    if (replaying || improveMode || !sessionId || !counting || sawDone) { setRunning(false); return; }
...
function start() {
  ...
  bindSocket(ws); // onmessage/onerror/onclose 統一掛載（含斷線自動重連）
}
