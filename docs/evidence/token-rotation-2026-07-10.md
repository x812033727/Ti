...
【驗證邊界聲明（勿誤讀）】：`--verify` 對「真實 token 生效」的 happy path **至今未驗證、待人工在場補做**。
本輪 AI 僅實跑不碰明文的 `--scan`／`--report`。對 `--verify` 的唯一自動化佐證是**間接**的——守門測試
`test_verify_without_token_fails_fast` 只證「缺 $GH_PAT 時 fail-fast 回非 0（正確拒絕）」，**未證**「持真 token
會正確通過」。切勿把「fail-fast 測試綠」誤讀為「--verify 已端到端驗證 OK」。
```
