...
| 2.9 | 其他秘密欄（OpenAI API Key／MiniMax API Key／GitHub Token） | 對 OpenAI API Key、MiniMax API Key、GitHub Token 各重複 2.5～2.8 的正常／空／超長檢查 | 行為與 Claude API Key 一致：可存、留空不清空、超長不破版、不顯示明文 | | | 秘密欄共用規則 |
