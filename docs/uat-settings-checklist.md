...
| 1.3 | 頁面載入與現值顯示 | 由上往下看面板裡的分組標題 | 依序出現分組標題：「一般」→「Claude」→「OpenAI」→「MiniMax」→「Gemini」→「Codex」→「Antigravity」→「混用（每角色 provider）」→「GitHub」→「並行」→「進階」 | | | |
| 1.4 | 頁面載入與現值顯示 | 看「一般」分組 | 出現「後端 Provider」下拉選單，且目前值（claude／openai／minimax／gemini／codex／antigravity）已被選取顯示 | | | |
