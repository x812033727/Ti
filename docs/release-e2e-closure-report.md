...
> 本報告引用 evidence 內 `body_sha256` 與落字規則，不另存報告端衍生雜湊。
...
```json
{
  "body_sha256": {
    ...
    "expected_from_evidence": "fd9b16d23eccafbd38d0d641585a025e2f77d98c8bce155b6d5a40648bf80dd4",
...
`body_sha256` 的計算規則已落在 `docs/evidence/release-v0.2.0-online-body.json` 的 `body_sha256_rule`：取 JSON 解析後的 `gh_release_view.body` 字串，直接以 UTF-8 bytes 計算 SHA-256；不得加 CLI/JQ 輸出換行，不得做 CRLF、尾空白、尾端空行等正規化；儲存格式為 64 字元小寫 hex。

可重現抽取指令採 `jq -rj '.gh_release_view.body' docs/evidence/release-v0.2.0-online-body.json | sha256sum`；`-j` 用來避免工具輸出自行補換行。報告只引用 evidence 內 `body_sha256=fd9b16d23eccafbd38d0d641585a025e2f77d98c8bce155b6d5a40648bf80dd4`，不在報告端另算，也不新增其他報告端衍生雜湊。
...
本報告已入 git 追蹤；2026-07-20 修復 `body_sha256` 計算方式時，同步更新 `docs/evidence/release-v0.2.0-online-body.json` 與 `docs/evidence/release-v0.2.0-online-reverify-2026-07-07.json` 的 exact hash 值與 `body_sha256_rule`，並更新本報告的三列表 #1、雜湊規則章與交付狀態章；未新增 evidence 副本或報告端衍生雜湊。
