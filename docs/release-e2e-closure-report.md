...
| #3 | `release: published` 實際觸發 release-smoke | `docs/evidence/release-smoke-v0.2.0-trigger.json` | `2026-07-05T18:24:35Z` | `run_id=27905531397`、`event=release`、`status=completed`、`conclusion=success`、`workflow_path=.github/workflows/release-smoke.yml` | 2026-07-06 成功；`gh run view` 可得欄位與 REST 全項 match。2026-07-07 線上重驗成功：實跑 `gh api repos/x812033727/Ti/actions/runs/27905531397 --jq '{id,event,status,conclusion,html_url,name,path}'`，以 `jq` 從 evidence 現值抽 expected 與本次 actual `diff -u` 指令化比對，無 mismatch；關鍵值：`run_id=27905531397`、`event=release`、`status=completed`、`conclusion=success`、`workflow_path=.github/workflows/release-smoke.yml`。易變欄位（updatedAt、下載計數等）只記錄於 evidence、不納入比對。`path` 在 `gh run view --json` 為 `N/A`，由 REST 補驗。自足抽取＋就地比對指令（鍵序皆字母排序對齊）：`diff -u <(jq -cS '{run_id,event,status,conclusion,html_url,workflow_name,workflow_path}' docs/evidence/release-smoke-v0.2.0-trigger.json) <(gh api repos/x812033727/Ti/actions/runs/27905531397 --jq '{run_id: "\(.id)", event, status, conclusion, html_url, workflow_name: .name, workflow_path: .path}')` | 不用 hash；以 GitHub Actions run metadata 勾稽。`path` 補驗指令：`gh api repos/x812033727/Ti/actions/runs/27905531397 --jq '{id,event,status,conclusion,html_url,name,path}'` |
