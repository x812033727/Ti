...
- **單獨跑** `.venv/bin/python -m pytest tests/test_qa_task1_server_boot.py` → **7 passed（全綠）**。
- **完整套件** `.venv/bin/python -m pytest tests/` → **8 failed**，且失敗**全部集中在 `test_qa_task1_server_boot.py`**：
...
3. `.venv/bin/python -m pytest tests/` 完整套件可乾淨綠燈（除既有的沙箱硬限制測試，如 bwrap/PID 隔離，需另記為已知跳過）。
