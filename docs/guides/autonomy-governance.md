...
- `POST /api/autonomy/brakes/{global|project}/clear`：管理員解除煞車並留下營運救援介入；明確解除
  per-project brake 會從該 audit 時點重算「連續失敗」期間，讓已修復根因的專案可恢復。
  每日成本、PR、rollback 等硬門檻仍以完整原始事件重算，不會因 clear 被洗掉。
