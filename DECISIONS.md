...
## 稽核資料流維持單向——清單第 27 列示例順序漂移改清單對齊實碼（`hooks → sandbox → cwd → model`），禁止反向改 `experts.py` 遷就清單
- 時間：2026-07-09 03:45
- 理由：程式碼為 SSOT、清單為被動鏡像，改清單對齊碼是唯一正解，保護依賴方向
- 否決方案：保留原樣＋加註理由——留著等於鏡像持續偏移，下輪還要再解釋一次，不划算

## #2 僅重排示例的具名參數順序，不變更 marker 字串本身（`experts._build_client` symbol 錨不動），確保 gate 的 `PYTHON_SOURCE_ANCHORS` 白名單零連鎖
- 時間：2026-07-09 03:45

## 示例參數順序不納入 AST 鎖定，僅列為移交待辦
- 時間：2026-07-09 03:45
- 理由：示例順序屬排版細節、非契約，用 AST 鎖之會把脆弱項升級成硬約束，違反可逆性優先
- 否決方案：順手鎖示例順序——超出本輪範圍且製造反向漂移風險

## 本輪不新增漂移偵測腳本、不新增守門測試——既有 5 支 lane baseline gate（AST 鎖欄位/16 列/13 marker）已覆蓋，驗證邊界＝`tests/docs` 為唯一 gate
- 時間：2026-07-09 03:45

## 修改後須 `git diff studio/docs` 確認只動第 27 列一行，未連帶重排其他列或碰欄位標題，避免「順手整理」擴大 diff
- 時間：2026-07-09 03:45
- 理由：高工提醒——控制 diff 面積，讓變更可精確回指本次漂移點

## 若既有 docs gate 含文字快照因該列變更而需同步 expected，一併更新並重跑；除此之外視為零連鎖
- 時間：2026-07-09 03:45
- 理由：工程師提醒的唯一潛在連鎖點，先驗證再收尾

## LaneContext 7 欄位、16 列對照表、13 marker 判定為零實質漂移，交付須貼出 `test_qa_task1_lane_baseline_inventory_live_audit.py` 跑綠結果自證對應，明講「已再核對、零漂移」，不接受口頭宣稱
- 時間：2026-07-09 03:45

## `lane manifest` 欄維持標註「前瞻契約、非現況」且全填「無」，待注入層落地再補鎖定欄位/優先序/fail-open|closed 的守門測試
- 時間：2026-07-09 03:45

## 收尾 `git status studio/docs tests/docs` 須無範圍外殘跡；文件互鏈類核心變更（若有）走 `x812033727/Ti` 獨立 PR，不混入本輪稽核產出
- 時間：2026-07-09 03:45
