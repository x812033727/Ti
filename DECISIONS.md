...
## `_COMMON` 在現有第 3 條後插入第 4 條，延續數字序列，不另起段落
- 時間：2026-07-11 15:30
- 理由：與現有結構一致、guard 測試子串比對最簡單；改字時測試立即報錯
- 否決方案：尾端獨立段落——序列斷裂，視覺上與前三條規則語義分裂

## 字數上限 500 字，明確定義適用範圍為「**單則發言的自由散文部分**」，結構化標記行（以 `任務:/驗證:/決議:/依賴:/後續任務:/核心改動:` 開頭）及其必要條列內容**均不計入**
- 時間：2026-07-11 15:30
- 理由：高工指出若只豁免 marker 開頭行，PM 拆解與 QA 條列說明的非 marker 散文仍受限，行為不可預期；明講「散文部分」才能讓模型正確套用
- 否決方案：泛稱「結構化輸出除外」——邊界模糊，不同角色模型解讀會分歧

## Guard 測試斷言子串取指示語意核心片段（如「自由散文部分」），**不含 `4.` 序號前綴**
- 時間：2026-07-11 15:30
- 理由：序號未來可能因規則重排而改；斷言應對內容敏感而非對編號敏感
- 否決方案：取含 `4.` 的前 15 字元——序號重排即測試誤報，維護成本高

## Guard test 斷言目標為 `roles.BUILTIN_ROLES`（import 期凍結全 8 角色），不用 `ROSTER`
- 時間：2026-07-11 15:30
- 理由：`ROSTER` 受 `config.OPTIONAL_ROLES` 影響，CI 環境可能只有 4 角色，覆蓋斷言不穩定

## 守門測試加在 `tests/core/test_roles.py` 尾端，不新增檔案、不新增 fixture
- 時間：2026-07-11 15:30

## `latency-improvement-report.md` 明寫「此為軟性 prompt 勸導，非 token 硬上限」，真環境補驗指令必須能量測加指示前後的**平均輸出 token 數與回應字數**（讀 `by_role.count/sum_ms`，並對比輸出 token 計數），不得讓讀者誤解 `~10ms/token` 效益已驗證
- 時間：2026-07-11 15:30

## 兩份 docs 歸 `docs/`；報告內的查核指令一律用 `timeout 60 bash -c ".venv/bin/python -c '...'"` 包裹，禁裸用 `python/python3/pytest`
- 時間：2026-07-11 15:30

## 改動範圍嚴格限三處——`studio/roles.py`（`_COMMON` 字串）、`tests/core/test_roles.py`（守門斷言）、`docs/`（兩份報告）；`config.py`、providers、discussion、experts 均不動
- 時間：2026-07-11 15:30
- 理由：可逆性最高——撤一行字串即回退；範圍小確保前後對比有效
