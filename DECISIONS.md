...
## 調用入口固定為 `python3 -m ruff`，不使用裸 `ruff`
- 時間：2026-06-21 01:36
- 理由：PATH 漂移可能取到非釘版的 ruff，`python3 -m` 保證使用當前 venv/python 環境下的 0.14.4
- 否決方案：裸 `ruff`（可攜性代價太高，版本飄移時靜默通過最危險）

## 驗收前必須執行 `python3 -m ruff --version` 確認回傳 `ruff 0.14.4`
- 時間：2026-06-21 01:36
...
## `git diff --stat` 無非預期改動作為硬閘門，與 `--check` exit=0 並列為必要條件
- 時間：2026-06-21 01:36
- 理由：CI 環境 `format` 有寫入行為；閘門缺席時「格式通過但悄悄動到其他檔」無法被偵測
- 否決方案：只看 `--check` exit=0（忽略副作用 diff，保護層缺一不可）

## 本輪四檔全數 exit=0 → 結論為「no-op 確認通過」，不開 PR、不產生 commit
- 時間：2026-06-21 01:36
- 理由：已格式化不需空 commit；空 commit 污染 git log 且誤導後續 reviewer
- 否決方案：建立確認用 commit（增加 log 噪音，且與「確認任務」定位不符）

## `pyproject.toml`、ruff 版本、`.pre-commit-config.yaml` 本輪一律不動
- 時間：2026-06-21 01:36
- 理由：三端（CI/pre-commit/本地）已對齊 0.14.4，任何版本變動需獨立評估影響範圍
- 否決方案：順手升版（會改變三端一致基線，超出本輪「確認任務」範疇）

## 不引入 Black 或任何其他格式化工具
- 時間：2026-06-21 01:36
