...
## 文件性質定位為「規範性契約（prescriptive）」，非描述既有實作
- 時間：2026-07-09 01:40
- 理由：LaneContext 現況只隔離 git worktree + 獨立專家團隊，無統一 env/manifest 注入層；不言明會讓文件看起來像已閉環，踩「假綠」教訓
- 否決方案：以描述既有實作（descriptive）語氣書寫

## 技術選型＝純文件變更，只動 ARCHITECTURE.md 一子段 + DECISIONS.md 一條 ADR，零程式碼、零新測試、零新依賴、不新增掃描規則
- 時間：2026-07-09 01:40

## 模組邊界＝ARCHITECTURE.md「任務並行」節 L107 後新增「baseline 注入契約」子段（不另開新章），DECISIONS.md 依既有時序 append 至檔尾，兩檔各單一寫入點
- 時間：2026-07-09 01:40
- 理由：子段內嵌維持 lane 敘事連貫、不製造孤兒段落
- 否決方案：另開獨立章節

## DECISIONS.md 新 ADR append 至檔尾，沿用既有「舊→新」單調時序
- 時間：2026-07-09 01:40
- 理由：現況首條 2026-06-13 最舊、往下堆新；插頂部會造成同檔雙時序並存，維護者易誤讀
- 否決方案：於檔案頂部新增 ADR

## 子段開頭第一句為 baseline 消歧句，明確區分本 baseline（lane 啟動設定基準：env + manifest）與 `runner.write_baseline_gitignore`（發佈前 .gitignore 淨化），同名不同源
- 時間：2026-07-09 01:40

## 子段開頭除消歧句外，加顯式狀態標籤「狀態：前瞻契約，lane 注入層尚未落地」
- 時間：2026-07-09 01:40
- 理由：狀態標籤比把「前瞻」埋進敘述更防半年後接手者誤當已實作

## 優先序寫成單一有序清單 `顯式注入 > env(TI_*) > lane manifest > 模組 DEFAULT`，註明與 `config.py`/`settings.py` 同一套語意（env 覆蓋檔案）
- 時間：2026-07-09 01:40
- 理由：對齊既有 SSOT 語意，避免另立 lane 專屬表述造成文件漂移
- 否決方案：為 lane 另立一套獨立優先序

## fail 策略分流準則＝安全/正確性關鍵項（密鑰、repo 路由、sandbox 開關）缺失/非法 → fail-closed（中止該 lane）；非關鍵增益項（可選旋鈕、可由 DEFAULT 推導者）→ fail-open（退回 DEFAULT + `log.warning`）
- 時間：2026-07-09 01:40

## 決策表為三欄（注入項類別｜缺失/非法時行為｜依據），每列標 fail-open/closed
- 時間：2026-07-09 01:40

## 決策表須含 `AUTOPILOT_REPO`→closed、`TI_DISCUSS_MODE`→open 兩個 repo 實例，並於表註聲明「此為 fail 策略之準則類比佐證，非 lane 注入層現有行為」
- 時間：2026-07-09 01:40
- 理由：不標「類比」會讓讀者誤以為 lane 注入層已按此跑，屬假閉環
- 否決方案：直接把兩實例當作 lane 注入層現況呈現

## ADR 沿用 DECISIONS.md 既有「## 標題／時間／理由／否決方案」四段格式，時間戳用今日日期，不引入 Nygard/MADR 新模板；否決方案至少記「env/manifest 一刀切同一 fail 策略」與「另開獨立章節」
- 時間：2026-07-09 01:40

## 雙向互鏈以「節標題文字」為錨（非行號）——ARCHITECTURE 子段末「見 DECISIONS.md『<ADR 標題>』」，ADR 內反指「ARCHITECTURE.md『任務並行』節 baseline 注入契約子段」
- 時間：2026-07-09 01:40
- 理由：跨檔行號 `tests/docs` 管不到、一改即死鏈；標題文字穩定可逆
- 否決方案：以行號建立互鏈

## 驗證邊界＝`tests/docs` 為唯一 gate，本輪不新增守門測試；決策表所述 fail-open/closed 行為尚無測試覆蓋，須於子段末與 ADR 明列為移交待辦「lane 注入層落地後補守門測試對齊決策表」（必要項，非裝飾）
- 時間：2026-07-09 01:40

## 路由歸屬＝ARCHITECTURE.md/DECISIONS.md 均屬 Ti 核心文件，此變更走 `x812033727/Ti` 核心 repo 獨立 PR，不混入專案 repo
- 時間：2026-07-09 01:40
