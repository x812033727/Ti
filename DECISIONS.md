...
## 本輪範圍鎖定「文件再稽核 + 防漂移」，只改 `studio/docs/lane_baseline_injection_inventory.md`，不建決策表、不落地 lane manifest、不改任何 symbol
- 時間：2026-07-09 02:52
- 理由：文件稽核膨脹成產品設計會讓 review 失焦、可逆性變差；決策表/manifest 是下一輪、走核心 repo 的獨立工作
- 否決方案：順手把決策表寫進 ARCHITECTURE/DECISIONS、或本輪一併落地 lane manifest

## 稽核資料流單向——程式碼為 SSOT、清單為被動鏡像；發現漂移一律改清單對齊碼，禁止反向改碼遷就清單
- 時間：2026-07-09 02:52
- 理由：保護依賴方向；`test_open_lane_and_worktree_startup_match_inventory` 已反向鎖原始碼片段（`env=`/`manifest` not in），雙向咬合是真閉環
- 否決方案：為讓清單好看而改動 orchestrator/runner symbol

## 清單 marker 以函式/類別 symbol 名為錨（如 `class LaneContext`/`async def _open_lane`），不用行號
- 時間：2026-07-09 02:52
- 理由：gate 用 AST `ast.get_source_segment` 抽 symbol 驗錨，行號跨檔無測試守護、一改即死鏈
- 否決方案：用行號或檔案偏移當錨

## 改動清單若動到 symbol 名，須同步更新 gate 的 `PYTHON_SOURCE_ANCHORS` 手維護白名單，收尾必跑 `tests/docs` 全綠確認錨點解析
- 時間：2026-07-09 02:52
- 理由：高工指出白名單是手維護的，改碼者容易漏改測試字典而非漏改清單，只看清單會假綠
- 否決方案：只核對清單內容、不跑測試就宣稱對齊

## `lane manifest` 保留為前瞻契約標註，對照表 `manifest 欄位` 欄全填「無」（由 `assert all(row[3]=='無')` 守住），並保留「現況＝顯式建構參數＋process-level config＋env 繼承、無 manifest 層」消歧句
- 時間：2026-07-09 02:52
- 理由：對齊業界「顯式 > env > manifest > DEFAULT」慣例並給下輪決策表錨點，同時擋住「誤讀成已落地」的假閉環
- 否決方案：刪掉 manifest 層只寫現況（下輪決策表失去錨點）

## `AUTOPILOT_REPO`→fail-closed、`TI_DISCUSS_MODE`→fail-open 兩個 repo 實例須明標為「fail 策略準則之類比佐證，非 lane 注入層現有行為」
- 時間：2026-07-09 02:52
- 理由：不標＝暗示 lane 注入已實作 fail 策略，屬假閉環；此點已有 `fail-closed 類比`/`不是 lane baseline 現有欄位` assert 背書
- 否決方案：直接把兩者寫成 lane 注入層現有 fail-open/closed 行為

## 驗證邊界＝`tests/docs` 為唯一 gate，本輪不新增守門測試；決策表 fail-open/closed 行為無測試覆蓋，列為移交待辦「lane 注入層落地後補守門測試對齊決策表」
- 時間：2026-07-09 02:52
- 理由：防漂移主要靠既有三支 gate + 人工 review，故文件措辭須直白無推論空間；缺口誠實暴露不粉飾
- 否決方案：假裝已有覆蓋、或本輪硬補與現況無對應的守門測試

## 交付須自證對應——零漂移時明講「已再核對、零漂移」並貼出跑綠的 `tests/docs` 結果或 diff，不接受口頭宣稱
- 時間：2026-07-09 02:52
- 理由：排除假綠；輸出要能回指本次核對，不是「看起來對」
- 否決方案：只口頭說零漂移、不附證據

## 決策表／ADR／ARCHITECTURE 互鏈屬 Ti 核心文件變更，走 `x812033727/Ti` 核心 repo 獨立 PR，不混入本輪稽核產出
- 時間：2026-07-09 02:52
- 理由：雙軌路由鐵則，核心框架文件不得混入專案 repo
- 否決方案：與本輪稽核清單同 PR 提交
