...
## 因五份為 QA 訊息流短碼（`778ced`/`rerun-765f1b` 等 repo grep 不到實體檔），權威檔採「單向明列作廢＋明文標註『訊息流明列值、非 repo 實體檔』」，雙向回指僅在原檔實際存在時才落地
- 時間：2026-07-08 06:57
- 理由：硬追形式雙向會逼我們憑空造五個檔或改歷史，兩者都違反 immutability；且不講白會讓終判卡在「作廢對象查無此檔」
- 否決方案：憑空造五個原檔以湊齊雙向連結

## 五份識別碼與省略 hash 逐字照抄 QA 訊息流，省略號（U+2026）原樣保留，落盤與測試斷言一律用 bytes 比對，防編輯器/格式化把 `…` 正規化成 `...`
- 時間：2026-07-08 06:57
