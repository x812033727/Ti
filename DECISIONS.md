...
## 本輪定案為「開 issue 規格 + 後續獨立 PR 移除」，不在本輪直接刪檔。
- 時間：2026-06-18 17:59
- 理由：保留審核與回滾邊界。
- 否決方案：順手 `git rm`。

## 技術選型維持純 `git` CLI + `rg`，不新增 wrapper、腳本或框架。
- 時間：2026-06-18 17:59
- 理由：一次性治理任務不值得增加長期維護面。

## Issue 前置證據必用只讀指令核對：`git show --stat 4572a49`、`git log -1 --oneline -- INVENTORY_task1.md`、`git ls-files --stage INVENTORY_task1.md`。
- 時間：2026-06-18 17:59
- 理由：避免把錯誤短 SHA 寫進任務規格。

## Issue 內保存完整 commit SHA，短 SHA 只作輔助顯示。
- 時間：2026-06-18 17:59
- 理由：長期追溯性優先。

## 引用搜尋固定用 `rg -n "INVENTORY_task1\.md|INVENTORY_task1" .`，並排除 `.git` 與建置輸出目錄。
- 時間：2026-06-18 17:59
- 理由：搜尋可重跑、可審核。
- 否決方案：手動列舉或模糊 grep。

## `INVENTORY_task1.md` 判定為一次性盤點/驗收基準文件，非產品文件。
- 時間：2026-06-18 17:59
- 理由：支持移除，但仍需 PR 化處理。

## 移除 PR 預設只做 `git rm INVENTORY_task1.md`，不得順手修改非必要檔案。
- 時間：2026-06-18 17:59
- 理由：控制 blast radius。

## 允許保留歷史/決策引用白名單：`DECISIONS.md`、`adr.json`、`VERIFICATION_REPORT_task4.md`，其餘引用需在 issue 中逐項判定。
- 時間：2026-06-18 17:59
- 理由：避免「引用清乾淨」誤傷決策紀錄。

## Issue 必附可複製指令：`git rm INVENTORY_task1.md`、`git status --short`、`git ls-files --error-unmatch INVENTORY_task1.md`、引用搜尋指令。
- 時間：2026-06-18 17:59

## 驗收的「工作目錄乾淨」只限定在移除 branch / PR 範圍，不拿他人既有 dirty worktree 當失敗。
- 時間：2026-06-18 17:59

## 若 PR 只刪此文件，只跑 git/rg 驗收；若改到 `README.md`、CI、程式碼或非純文件，需加跑受影響範圍檢查。
- 時間：2026-06-18 17:59
- 理由：驗證成本跟風險綁定。

## 依賴方向固定為證據核對 -> 文件屬性判定 -> issue 規格 -> 獨立 branch/PR 移除 -> 驗收。
- 時間：2026-06-18 17:59
- 否決方案：跳過 issue 直接清理。
