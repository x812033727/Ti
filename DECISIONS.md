...
## **裸直譯器命令邊界（精修版）**：以「**前一個字元非 `[\w/.-]` 且後接 `\s` 或 `-m` 邊界**」為統一判準（regex 與文字描述對齊），覆蓋 shell 行首呼叫、行內 `xargs python` / `$(which python)` / `command -v python` 等所有「被 shell 直譯器呼叫」情境。
- 時間：2026-06-15 21:19
- 理由：統一 regex 與文字判準，消除 reviewer/實作者兩套標準；`[^\w/.-]` 比 `[\s\`$]` 多擋 `xargs`/`$()`/`command -v` 漏網情境。
- 否決方案：原始寫法「行內以空白/反引號/`$` 起首」——會漏 `xargs python` 等案例。

## **豁免採雙軌制**：整檔白名單（`Dockerfile`、`*.lock`、純 manifest 標頭）＋行內語法豁免（套件 manifest 的 `^[a-zA-Z_-][\w.-]*\s*=\s*`、Docker image tag 行 `FROM\s+\S*python\S*`、venv 執行檔路徑 `.venv/bin/python` / `.venv\Scripts\python`、shebang 行 `^#!.*\bpython\b`）。
- 時間：2026-06-15 21:19
- 理由：純檔案白名單會把 `pyproject.toml` 整檔豁免，等於放棄對它的守護；純行內黑名單日後加新檔案會靜默漏抓。雙軌讓檔案級豁免可審核、套件語法不會被誤抓。
- 否決方案：純檔案白名單（丟失 `pyproject.toml` 守護）；純行內黑名單（不可審核、新檔案易漏）。

## **執行期零變動，但 pre-flight 先驗**：`scripts/publish_release.py` 與 release 流程、gh CLI 旗標、既有守護測試符號**全部不動**；但動工前先 `rg -n 'subprocess|sys.executable' scripts/` 與 `rg -n '\bpython\b' scripts/publish_release.py` 跑一次，確認命中僅為 shebang/註解/字串，無執行期呼叫。
- 時間：2026-06-15 21:19
- 理由：動工前先有 ground truth，避免「決策寫零變動但實作發現有 subprocess 呼叫」被迫破例。
- 否決方案：直接動工事後補救（一旦撞到 `subprocess.run(['python', ...])` 要回頭改設計，PR 噪音更大）。

## **守護測試擴充 `no_bare_python_command` 斷言**，歸入 `tests/docs` 既有模組；斷言內顯式宣告雙軌豁免規則，命中清單（rg 輸出）為空才 PASS。
- 時間：2026-06-15 21:19
- 理由：與既有「`-F` 必須在 create-release step run 內」、「`create-release` step id」同級守護，失敗模式具體；命中清單可一眼審核。
- 否決方案：另開 `tests/check_no_bare_python.py`（與既有 `tests/docs` 風格分裂）。

## **CONTRIBUTING.md 新增「Python interpreter convention」一節**，慣例主因寫實務理由（`python3` 在三大平台都是預設可用的 3.x 進入點），PEP 394 改為附帶參考（標 deprecated）。
- 時間：2026-06-15 21:19
- 理由：PEP 394 於 2024-05 已標 deprecated，文件若寫「PEP 規定」會過期；實務理由更耐維護。
- 否決方案：引用 PEP 394 作為主因（會過期）。

## **README Quickstart 採「主指令單一 + 雙句退路」**：主範例 `python3 app.py` 一行；其下加兩句「Windows 若 `python3` 找不到，可改用 `py`」「`py` 啟動器預設跑最新版，想鎖 3.x 用 `py -3`」。
- 時間：2026-06-15 21:19
- 理由：退路不只給 `py`，要順手講清 `py` vs `py -3` 行為差，避免使用者踩雷。
- 否決方案：雙範例並列（認知負擔，且暗示兩者等價）。

## **不重建既有 `.venv`、smoke 採 fresh venv + 前置檢查**：用 `command -v python3 || { echo "FATAL: python3 not on PATH"; exit 1; }` 確認 `python3` 可用後，跑 `python3 -m venv .fresh-venv && .fresh-venv/bin/python --version` 證明 venv 內 `python` 仍可運作。
- 時間：2026-06-15 21:19
- 理由：`.fresh-venv/bin/python` 預設依賴 `python3` 在 PATH；前置檢查避免「連 `python3` 都沒有」時假綠。
- 否決方案：直接 `.fresh-venv/bin/python --version`（環境無 `python3` 時假綠）；動既有 `.venv`（擴大變更半徑）。

## **驗收三層、任一失敗即退回**：(1) 字串層 `rg -n '(^|[^\w/.-])python(\s|-m\b)' --glob '!*.lock'` 在豁免清單外零命中；(2) 測試層 `python3 -m pytest tests/docs -q` 全綠；(3) 執行層依文件指令實跑 demo 第一步，無 `command not found`。
- 時間：2026-06-15 21:19
- 理由：三層分別守「字串漂移」「守護測試整合」「end-to-end 真實可用」；任一失效都能定位具體失敗模式。
- 否決方案：只跑「rg + pytest」省去實跑（驗不到 shell 環境差異）；三層之上再加第四層（如 lint）——無需求、徒增維護。

## **CHANGELOG 與 PR 描述留一筆**：「文件慣例改用 `python3` 為裸直譯器命令；豁免清單見 CONTRIBUTING 慣例節」，註明 PR 編號與 `tests/docs::test_no_bare_python_command` 守護測試位置。
- 時間：2026-06-15 21:19
- 理由：慣例轉換半年後看 git log 看不出為什麼改，CHANGELOG 留筆比 PR 描述更耐翻。
- 否決方案：只在 PR 描述寫、CHANGELOG 不提（PR 描述易被埋沒）。

## **單一 PR 一次到位**；若 `README.md` 改法（大量段落）與其他檔案（單行 `python script.py`）差異過大，拆兩個 commit 同一 PR，便於 reviewer 分階段看。
- 時間：2026-06-15 21:19
- 理由：邊界清楚、豁免顯式，分批只增加 review 噪音；同一 PR 內拆 commit 兼顧審核便利。
- 否決方案：拆多個 PR（增加 review 噪音、無實質價值）。

## **手動掃一次 README 其他章節**（Development、Troubleshooting、FAQ 等），確認所有 shell 範例都收斂到 `python3`，不留「同份文件兩套慣例」。
- 時間：2026-06-15 21:19
- 理由：守護測試的 `*.md` glob 會擋下未來漂移，但既有檔案內的漂移需本次 PR 順手處理。
- 否決方案：只掃 Quickstart 段、其他章節交給未來 PR（漂移累積、後續清更痛）。

## **裸直譯器命令邊界**：以「**前一個字元非 `[\w/.-]` 且後接 `\s` 或 `-m` 邊界**」為統一判準，覆蓋 shell 行首呼叫、行內 `xargs python` / `$(which python)` / `command -v python` 等所有「被 shell 直譯器呼叫」情境。
- 時間：2026-06-15 21:19
- 理由：regex 與文字描述必須對齊到同一句；`[^\w/.-]` 比 `[\s\`$]` 多擋 `xargs`/`$()`/`command -v` 漏網情境。
- 否決方案：原始寫法「行內以空白/反引號/`$` 起首」——文字與 regex 兩套標準，且漏 `xargs python` 等案例。

## **執行期零變動，但動工前先驗**：`scripts/publish_release.py` 與 release 流程、gh CLI 旗標、既有守護測試符號**全部不動**；動工前先 `rg -n 'subprocess|sys.executable' scripts/` 與 `rg -n '\bpython\b' scripts/publish_release.py` 跑一次，確認命中僅為 shebang/註解/字串，無執行期呼叫。
- 時間：2026-06-15 21:19
- 理由：先有 ground truth 再動檔，避免撞到 `subprocess.run(['python', ...])` 後被迫破例回頭改設計。
- 否決方案：直接動工事後補救（PR 噪音更大）。

## **守護測試擴充 `tests/docs::test_no_bare_python_command`**，斷言內顯式宣告雙軌豁免規則，rg 命中清單為空才 PASS。
- 時間：2026-06-15 21:19
- 理由：與既有「`-F` 在 create-release step run 內」、「`create-release` step id」同級守護，失敗模式具體可定位。
- 否決方案：另開 `tests/check_no_bare_python.py`（與既有 `tests/docs` 風格分裂）。

## **CONTRIBUTING.md 新增「Python interpreter convention」一節**，慣例主因寫實務理由（`python3` 在三大平台都是預設可用的 3.x 進入點），PEP 394 改為附帶參考並標 deprecated。
- 時間：2026-06-15 21:19
- 理由：PEP 394 於 2024-05 已標 deprecated，文件若寫「PEP 規定」會過期；實務理由更耐維護。
- 否決方案：引用 PEP 394 作為主因（會過期）。

## **README Quickstart 採「主指令單一 + 雙句退路」**：主範例 `python3 app.py` 一行可複製；其下加兩句「Windows 若 `python3` 找不到，可改用 `py`」「`py` 啟動器預設跑最新版，想鎖 3.x 用 `py -3`」。
- 時間：2026-06-15 21:19
- 理由：退路不只給 `py`，要順手講清 `py` vs `py -3` 行為差。
- 否決方案：雙範例並列（認知負擔，且暗示兩者等價）。

## **不重建既有 `.venv`、smoke 採 fresh venv + 前置檢查**：先 `command -v python3 || { echo "FATAL: python3 not on PATH"; exit 1; }` 確認 `python3` 可用，再 `python3 -m venv .fresh-venv && .fresh-venv/bin/python --version` 證明 venv 內 `python` 仍可運作。
- 時間：2026-06-15 21:19
- 理由：`.fresh-venv/bin/python` 預設依賴 `python3` 在 PATH；前置檢查避免「連 `python3` 都沒有」時假綠。
- 否決方案：直接跑 venv 驗證（環境無 `python3` 時假綠）；動既有 `.venv`（擴大變更半徑）。

## **單一 PR 一次到位**；若 `README.md` 改法與其他檔案差異過大，拆兩個 commit 同一 PR，便於 reviewer 分階段看。
- 時間：2026-06-15 21:19
- 理由：邊界清楚、豁免顯式，分批只增加 review 噪音；同一 PR 內拆 commit 兼顧審核便利。
- 否決方案：拆多個 PR（增加 review 噪音、無實質價值）。

## **手動掃一次 README 其他章節**（Development、Troubleshooting、FAQ 等），確認所有 shell 範例都收斂到 `python3`，不留「同份文件兩套慣例」漂移。
- 時間：2026-06-15 21:19
- 理由：守護測試的 `*.md` glob 會擋下未來漂移，但既有檔案內的漂移需本次 PR 順手處理。
- 否決方案：只掃 Quickstart 段、其他章節交給未來 PR（漂移累積、後續清更痛）。
...
## 採雙頻道合約——機器頻道（`[tag]` 括號前綴）鎖在 gate 回傳、`set_status(note=)`、`add(detail=out[-500:])`；人類頻道（自然語言「修復 lint 失敗／修復缺 SDK collection／修復測試失敗」）鎖在 `add(title=)`。兩頻道刻意不強制同語法。
- 時間：2026-06-16 11:46
- 理由：機器頻道服務 grep / log 解析 / AST / 自動化；人類頻道服務人眼 / backlog UI / 決策。兩者受眾不同，語法解耦避免把機器語法灌進人眼欄位。
- 否決方案：單頻道（所有欄位統一 `[tag]`）——成本低但人眼欄位訊息密度下降（失去「修復」「失敗」語意），且當前 3 處標題已用自然語言，統一等於 churn。

## 不在 `add(title=)` 標題補 `[tag]` 前綴，本任務 0 行業務邏輯改動。
- 時間：2026-06-16 11:46
- 理由：標題已含自然語言層級（人眼可辨）；機器端三個入口（return / note / detail）已覆蓋完整；翻案成本 = 3 個 f-string + 1 條新測試，可逆性極高。現在補屬鍍金。
- 否決方案：移除標題內自然語言層級字（只留 `f"修復：{task['title']}"`）——更糟，連人眼都失去層級。

## 新增 1 條反向鎖定測試 `test_fix_task_title_has_no_bracket_prefix`，驗證三處 `backlog.add(f"修復 X 失敗：...")` 標題字串不含 `[` 或 `]`。
- 時間：2026-06-16 11:46
- 理由：docstring 約定會漂；反向測試把「標題不應有括號前綴」轉成 CI 可執行的契約，未來誰手賤加前綴會直接紅。成本 < 15 行、零風險，與既有 14 條同檔。
- 否決方案：只靠 test docstring 約定（工程師原提議）——docstring 是人讀文件，不會跑；6 個月後新人直接改標題 PR 過了，契約實質失效。

## 在三處 `backlog.add(f"修復 X 失敗：...")` 上方各加 1 行 inline 註解 `# 標題頻道：自然語言（見模組 docstring 雙頻道合約）`。
- 時間：2026-06-16 11:46
- 理由：讀 `autopilot.py` 先看到程式碼才看測試；三處 inline 註解是半年後接手者最直接看到的合約提醒。成本 3 行。
- 否決方案：不加 inline 註解——依賴讀者先看模組 docstring 再看程式碼，實務上不會發生。

## 將雙頻道合約文字**同步**寫進 `studio/autopilot.py` 模組 docstring，明列：閘門清單（lint/collect/test）、機器頻道位置（return/note/detail）使用 `[tag]`、人類頻道位置（fix task title）使用自然語言「修復 X 失敗：...」、翻案觸發條件（backlog UI 只秀標題、LLM regex 評估層級、grep 一致性需求）。
- 時間：2026-06-16 11:46
- 理由：模組 docstring 是讀代碼第一站，比 test docstring 觸及率高；決策理由落字在源碼旁，git log 找得到。成本 1 段（5–8 行）。
- 否決方案：僅寫在 test docstring——測試 docstring 是測試開發者看的，模組 docstring 才是所有人看的；前者覆蓋面太窄。

## 在 test docstring 補一句「閘門順序 = 源碼陳列順序；新增閘門時請同步補對應測試」；同步承認此為 loud failure（非 silent）——重排閘門時 `_gate_failure_branches()[:3]` 會讓斷言錯位而 fail，不會沉默通過。
- 時間：2026-06-16 11:46
- 理由：顯性化隱性假設，下個接手者加第 4 個閘門時不會漏測試；並校正風險性質（loud failure 而非 silent corruption）以匹配實際行為。
- 否決方案：在本任務修補 AST helper 用 `out = await _gate_X(clone)` 精確配對——成本約 10–15 行、需重驗 14 條合約、scope 飄移；當前 loud failure 已能攔下重排，沒事修等於製造風險。

## AST 順序耦合（`_gate_failure_branches()[:3]` 依賴源碼陳列順序）flag 為已知脆弱性、不在本任務修補、列為後續待辦。
- 時間：2026-06-16 11:46
- 理由：當前 14 條測試全綠、源碼順序是事實、沒人會調 gate 順序；動 AST helper 等於重驗合約，scope 飄移。
- 否決方案：本任務順手修——會擴大本 PR 的 diff 範圍、需重跑守護測試，違反「本任務 0 行業務邏輯改動」原則。

## 不在當前實作 helper `_gate_label(name: str) -> str`；保留為「若日後翻案補前綴」時的配套進場項。
- 時間：2026-06-16 11:46
- 理由：三處字串重複目前是事實的，helper 化屬提早一般化（YAGNI）；日後若要加第 4 個閘門、或標題翻案補前綴需集中管理時，再 helper 化。
- 否決方案：當下 helper 化——3 處字串飄移風險目前靠 inline 註解 + 模組 docstring + 反向測試三道鎖已足，helper 化屬 over-engineering。

## 不在 `add(title=)` 標題補 `[tag]` 前綴；本任務 0 行業務邏輯改動。
- 時間：2026-06-16 11:46
- 理由：標題已含自然語言層級（人眼可辨）；機器端三個入口（return / note / detail）已覆蓋完整；翻案成本 = 3 個 f-string + 1 條新測試，可逆性極高。現在補屬鍍金。
- 否決方案：標題補前綴（`f"[lint] 修復 lint 失敗：..."`）——強化 grep 一致性但人眼欄位訊息密度下降，且需擴充現有 14 條測試覆蓋標題路徑。

## 將雙頻道合約文字同步寫進 `studio/autopilot.py` 模組 docstring，明列：閘門清單（lint/collect/test）、機器頻道位置（return/note/detail）使用 `[tag]`、人類頻道位置（fix task title）使用自然語言「修復 X 失敗：...」、翻案觸發條件（backlog UI 只秀標題、LLM regex 評估層級、grep 一致性需求）。
- 時間：2026-06-16 11:46
- 理由：模組 docstring 是讀代碼第一站，觸及率高於 test docstring；決策理由落字在源碼旁，git log 找得到。成本 1 段（5–8 行）。
- 否決方案：僅寫在 test docstring——測試 docstring 服務測試開發者，模組 docstring 服務所有人；前者覆蓋面太窄，回應高級工程師 #2 補強。

## 在 test docstring 補一句「閘門順序 = 源碼陳列順序；新增閘門時請同步補對應測試」；同步重述此為 loud failure（非 silent）——重排閘門時 `_gate_failure_branches()[:3]` 會讓斷言錯位而 fail，不會沉默通過。
- 時間：2026-06-16 11:46
- 理由：顯性化隱性假設；下個接手者加第 4 個閘門時不會漏測試；校正風險性質匹配實際行為。成本 1 行。
- 否決方案：在本任務修補 AST helper 用 `out = await _gate_X(clone)` 精確配對——成本約 10–15 行、需重驗 14 條合約、scope 飄移；當前 loud failure 已能攔下重排，沒事修等於製造風險。

## 不在當前實作 helper `_gate_label(name: str) -> str`；保留為「若日後翻案補前綴」或「加第 4 個閘門」時的配套進場項。
- 時間：2026-06-16 11:46
...
## **技術選型：純前端 vanilla JS 改動，`web/app.js` 無 build step。** 後端 payload / orchestrator / `studio/events.py` 不動。
- 時間：2026-06-17 19:06
- 理由：問題在 `web/app.js:325-339` 渲染處漏讀 `criteria`；後端已送對的欄位（payload docstring 載明），修補面收斂在前端是 scope 最小。
- 否決方案：(α) 改 orchestrator 注入「policy option」新欄位——scope 飄移，且猜測語意源頭；(β) 改 payload schema——破既有合約、影響 history 重播。

## **「警示塊政策選項」推定為 `agenda` 子題的 `criteria` 欄位。** 翻案條件：**(a)** 工程師 #1 證實 orchestrator 實際未填 criteria；**(b)** PM 確認「政策選項」是另一個語意實體（PM 才是語意源頭，非工程師推定）。
- 時間：2026-06-17 19:06
- 理由：(a) 既有 `agenda_plan` docstring 明列「含每子題 title/description/criteria/assignee」；(b) 語意上「成功準則」=「政策選項／判斷準則」最貼近 PM 描述；(c) 症狀對齊：進 history 不見於畫面。
- 否決方案：(a) `tasks` / `edges`——任務依賴語意、非政策語意；(b) `corrections`——已渲染（app.js:333-335）。

## **渲染形式採「拼到子題行尾 + 【準則】標籤」**——沿用既有模板追加 `if (a.criteria) line += '｜【準則】${a.criteria}';`，`｜` 與既有 description separator 一致、`【】` 與既有 `conclusion` case（app.js:357）標籤風格一致。
- 時間：2026-06-17 19:06
- 理由：與既有 `title / description / assignee` 內聯拼裝同構，零 CSS、零結構破壞、可逆性最高。
- 否決方案：(a) 多行縮排獨立區塊——視覺重但需新 CSS、破既有 DOM 結構；(b) 改用模板引擎——scope 飄移到前端基礎設施；(c) 在 `addSystem` 內另外呼叫 `addMessage`/自訂 renderer——多型別開銷、不符既有慣例。

## **`criteria` 為空 / 缺欄位 / 空字串時靜默省略**——`if (a.criteria)` 守衛。沿用既有 `if (a.description)` / `if (a.assignee)` 慣例。
- 時間：2026-06-17 19:06
- 理由：既有契約「缺漏即不顯示」一致；避免「【準則】（無）」噪音。
- 否決方案：(a) 顯式「【準則】（無）」——視覺噪音、破壞既有渲染契約；(b) 區分空字串 vs undefined 用 `?? ''`——過度細緻、現實中 orchestrator 不會塞空字串佔位。

## **守護鏈三層**：(i) **新增源碼靜態掃描測試**（鎖定 agenda_plan case 區塊，見決策 6）；(ii) **既有 `test_offline_agenda_e2e` 守 payload 結構**（不覆蓋 criteria 渲染內容、僅守 agenda/assignments/corrections/tasks schema 正確）；(iii) **人眼 demo**（由工程師交付前自測）。
- 時間：2026-06-17 19:06
- 理由：本任務為「前端渲染修補」，測試守護是「半年後接手者還能信任這條路徑」的安全網。**自我認知修正**：既有 e2e **不**模擬前端 replay、**不**檢查 DOM 文字——設計原以為的 e2e 覆蓋在 criteria 渲染層面是缺口，本測試 + 人眼 demo 才是當前實際覆蓋。**commit message 與測試 docstring 明示此缺口**。
- 否決方案：(a) 升級到 jsdom——需 npm 依賴 + package.json，scope 飄移到前端基礎設施層；(b) 升級到 node 直譯 `app.js`——`app.js` 依賴 `document.querySelector` 等瀏覽器 API，mock DOM 成本等同 jsdom。

## **守護測試形式：鎖定 `case "agenda_plan":` 區塊的靜態正則掃描**。用 regex 抓出 `case "agenda_plan":` 到下一個 `case "` 為止的 case 區塊字串，在該區塊內斷言。**正樣**：`a.criteria` 與 `${a.criteria}` / `line += ... criteria` 模式同時出現（擋掉「守衛寫了但忘記串到 `line`」）；**負樣**：`a.tasks` 與 `a.edges` 不應在該區塊內出現（守住「不渲染 tasks/edges」範圍守門，避免 scope 飄移）。
- 時間：2026-06-17 19:06
- 理由：工程師 push back 採納——裸 grep `a.criteria` 判別力太弱，鎖定 case 區塊是 (β-lite) 強化版：純 python regex、零新基礎設施、真有判別力（擋掉「if 寫了但沒接字串」）。負樣改用 `a.tasks` / `a.edges` 取代虛構的 `policy_option` 拼字——有實質範圍守門意義，不是假想敵。
- 否決方案：(a) 裸 grep `a.criteria`——判別力弱、可被 `if (a.criteria) line += ''` 騙過；(b) jsdom / node 直譯——scope 飄移；(c) 負樣用 `policy_option` 拼字——虛構、orchestrator 從未用、永遠綠等於沒守。

## **不抽 helper、不重構既有 `agenda_plan` case 結構**——與既有 `title/description/assignee` 內聯拼裝同構。
- 時間：2026-06-17 19:06
- 理由：沿用既有架構決策 7（YAGNI）；未來若 case 整段要拆函數再一起抽，本次局部改動不越界。
- 否決方案：(a) 抽 `_renderAgendaItem()` helper——提早一般化，本檔只一處使用；(b) 改模板字串模板引擎——scope 飄移。

## **commit 粒度：3 檔變更單一 commit，耦合意圖**——(1) `web/app.js` modify（`agenda_plan` case 加 criteria 渲染）；(2) 新檔 `tests/qa_test_render_agenda_criteria.py`；(3) `tests/test_verify_clean_acceptance.py` 的 `ignored_untracked` set +1 元素（加入 `tests/qa_test_render_agenda_criteria.py`）。**翻案既有架構決策 5**（從「不擴充」改為「擴充 1 個元素」），理由：與前一個任務的翻案對稱處理——新測試在「建檔後、未 commit 前」的中間態會以 `??` 出現、被 `test_no_untracked_residuals_in_worktree` flag 為「未預期 untracked 殘留」；屬「QA 工具造成的可預期污染」白名單維護、非業務邏輯改動，對稱處理是合約對齊的最小改動。commit message: `feat(web): agenda_plan 渲染補上 criteria 成功準則（前端可見、history 已有）` + `Refs: 新增守護測試在 commit 前為 untracked，ignored_untracked 對稱處理；既有 e2e 不覆蓋 criteria 渲染，執行時守護缺口由人眼 demo 補上`。
- 時間：2026-06-17 19:06
- 否決方案：(a) 拆 2 個 commit——拆 commit 必有中間態 fail（`ignored_untracked` 未擴充時新測試檔被 flag），單一 commit 才能保持工作樹始終綠；(b) 用驗收順序流程紀律取代白名單——半年後接手者照著跑會踩雷、白名單維護成本極低；(c) 不擴充白名單、要求工程師 commit 後才跑 `test_verify_clean_acceptance.py`——脆弱、易迴歸。

## **測試 docstring 明列不守護事項 + 翻案條件**。「**守護**：`web/app.js` 的 `agenda_plan` case 區塊內 `a.criteria` 確實讀取並串到 `line` 賦值，且 `a.tasks` / `a.edges` 未進入該區塊（範圍守門）。**不守護**：(i) orchestrator 是否實際填入 criteria（屬後端範疇，本任務不動）；(ii) DOM 在瀏覽器實際呈現（屬 e2e 範疇，由人眼 demo 與既有 `test_offline_agenda_e2e` payload 結構檢查補上，但**既有 e2e 不模擬前端 replay**）；(iii) criteria 為空字串時的省略行為（沿用既有 `if (a.description)` 守衛慣例，由 code review 守護）；(iv) 既有 title/description/assignee/corrections 不迴歸（既有 `test_offline_agenda_e2e` 守護）。**翻案條件**：(a) orchestrator 證實未填 criteria；(b) PM 確認「政策選項」是另一個語意實體。」
- 時間：2026-06-17 19:06
- 理由：避免半年後接手者誤判守護範圍；明示執行時守護缺口是誠實的設計自我認知。
- 否決方案：(a) 只列「守護事項」、不列「不守護事項」——誤導接手者以為覆蓋完整；(b) 在 docstring 寫 jsdom 升級路徑——scope 飄移暗示，不應綁死本任務。

## **本機驗收順序**：`ruff check tests/qa_test_render_agenda_criteria.py tests/test_verify_clean_acceptance.py`（`web/app.js` 非 python 跳過 ruff）→ `ruff format --check` → `pytest -v tests/qa_test_render_agenda_criteria.py`（單檔先綠）→ `pytest -v tests/test_offline_agenda_e2e.py`（既有 e2e 不迴歸）→ `python3 -m pytest -v tests/test_offline_agenda_e2e.py tests/ -k "agenda"`（任務指定集合）→ `git diff --cached --stat`（確認 staged 為 3 檔）→ `git status --porcelain`（手動確認無新 `??`）→ commit → `git status --porcelain`（commit 後無 `??`）→ `pytest -q`（含 `test_verify_clean_acceptance.py` 確認白名單生效）→ 整體 `pytest -q` 防迴歸。**任一步紅則不 commit**。
- 時間：2026-06-17 19:06
- 理由：步驟排序對應「白名單議題已被決策 8 吸納」的狀態——`test_verify_clean_acceptance.py` 在 commit 後跑才驗證白名單生效。
- 否決方案：(a) 把「commit 後跑 `test_verify_clean_acceptance.py`」寫進測試本體——污染職責、由 git status 手動確認 + 流程紀律已覆蓋。

## **範圍守門（明確不做）**：(a) 不改 orchestrator / 後端 / payload 結構；(b) 不渲染 `tasks` / `edges`（任務依賴圖層級，需新 CSS 與新排版，scope 飄移；由負樣斷言守住）；(c) 不重構既有 `agenda_plan` case 結構；(d) 不動 `corrections` 既有渲染；(e) 不建新注入管線、不改 history jsonl 結構、不引入 jsdom / node 測試基礎設施。
- 時間：2026-06-17 19:06
- 理由：PM 已明示「最小且可收斂」+「人眼可見」即驗收；擴張會破壞決策可逆性、把單次修補變成前端基礎設施工程。
