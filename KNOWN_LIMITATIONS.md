...
- [x] 補/確認 guard 測試，鎖住「退避秒數計算與錯誤文字分類器唯一實作在 `llm_caller`」，任一呼叫端複製手寫即變紅 —— `tests/scan/test_retry_single_home_guard.py`：退避公式 `2**attempt` 限 `llm_caller.py`＋（carve-out）`publisher._backoff`（git/CI 重試、與 LLM 限流正交）；分類標記 `overloaded_error` 限 `llm_caller.py`；含自證/排除假綠對照樣本
