...
- 原**無執行期預算上限/熔斷**：唯一預算是 autopilot 的**時間**預算。互動場（web/ws）跑飛了只能撞硬 timeout。

> **已落地（成本熔斷）**：新增每場用量預算 `TI_SESSION_TOKEN_BUDGET`／`TI_SESSION_USD_BUDGET`
> （`config.py`，0=不限、預設關），與時間預算共用同一條優雅收尾路徑。`StudioSession._counting_broadcast`
> 在事件單一收斂點即時累計 token／成本，`_budget_exceeded()` 達上限即觸發 `_should_wind_down()`——
> 停止派發新任務、以已完成成果出貨、發「用量預算收斂」事件（與時間版區分措辭）。涵蓋全 lane／reviewer。
> 守護：`tests/test_session_usage_budget.py`（累計、token/USD 門檻、預設關、整合熔斷收尾）。
...
### 3.3 retry 公開介面無 guard（P0，低風險）✅ 已落地

`KNOWN_LIMITATIONS.md` 原自承：retry 系統公開介面（`llm_caller` 導出）尚未用 guard 測試鎖定。
風險：任何呼叫端複製手寫 retry／繞過 `make_retry_config()` 工廠不會變紅，會悄悄破壞「單一退避
真相 + 禁疊乘」的鐵則。

> **已落地**：`tests/scan/test_retry_single_home_guard.py`——靜態守護退避公式 `2**attempt` 只能在
> `llm_caller.py`（＋明確 carve-out `publisher._backoff`，git/CI 重試與 LLM 限流正交），分類標記
> `overloaded_error` 只能在 `llm_caller.py`；含自證對應＋排除假綠的黑/白對照樣本。任一呼叫端複製
> 手寫即變紅。`KNOWN_LIMITATIONS.md` 對應項已勾掉。
...
| 項目 | 預期效益 | 風險 | repo | 狀態 |
|------|---------|------|------|------|
| Prompt caching 稽核 → 改量測快取成效（cache_hit） | 從「不知有沒有用」變可量測 | 低 | 核心 | ✅ 已落地（§1.3） |
| 每場 token/USD 軟預算 + 優雅收尾 | 止住失控場燒錢撞 timeout | 低 | 核心 | ✅ 已落地（§1.4） |
| retry 公開介面 guard 測試 | 鎖死「單一退避真相」 | 低 | 核心 | ✅ 已落地（§3.3） |
| `config` 降本預設 A/B（用 usage_report 定案） | -30~50% 成本 | 低 | 核心 | ⏳ 營運調參：旋鈕＋cache_hit 儀表已備，待累積真實數據定案（§1.5） |
