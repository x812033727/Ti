...
| 3 | `autopilot.py:87` (`_gate_tests`) | `python -m pytest -q`（**已遷移 exec**） | **a** | 固定字串，無 shell 語法 | ✅ 已遷移：argv `["python","-m","pytest","-q"]` 走 `run_command_exec`，`label="pytest gate"`；保留 `timeout=600, sandbox=True`。註：shell→exec 皆從 PATH 解析 `python`，遷移未改變可解析性 |
