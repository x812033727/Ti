...
# Email 推播：SMTP host + 收件人皆非空才啟用；與 webhook/Telegram 並存，各自獨立成敗。
...
ALERT_FROM = os.getenv("TI_ALERT_FROM", "Ti Studio <noreply@localhost>").strip() or (
    "Ti Studio <noreply@localhost>"
)
...
# SLO 自動煞車(第 3 階 A4):7 天零人工介入合併率(insights.trust_metrics)低於此門檻時,
# 自產日額自動砍半並推播 slo_brake(每 UTC 日至多一次)。0=停用(預設;信任基線收完、
...
# 意圖迴路(第 4 階 B3):improver「找問題」先對照專案常駐 intent 做差距分析,自產任務
...
def reload() -> None:
    ...
    INTENT_LOOP = os.getenv("TI_INTENT_LOOP", "0") not in ("0", "false", "False", "")
    INTENT_DISCOVERY = os.getenv("TI_INTENT_DISCOVERY", "0") not in ("0", "false", "False", "")
    PROJECT_IMPROVE_AUTO = os.getenv("TI_PROJECT_IMPROVE_AUTO", "0") not in (
        "0",
        "false",
        "False",
        "",
