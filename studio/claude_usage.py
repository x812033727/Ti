...
暫時性失敗（連線錯誤／429 Too Many Requests／5xx／壞 JSON）另有 last-known-good 回退：
15 分鐘內查成功過就回舊值（標 ``stale: True``、保留原 fetched_at），而非 ``unreachable``
全 None——否則一次 429 會讓 autopilot 把該帳號判成「不可用」而強制切走（帳號輪替誤判）。
授權類失敗（401/403／token 缺失／已知過期）不回退：舊值掩蓋不了「token 壞了」的事實。
"""
...
# last-known-good：{憑證檔路徑字串: (fetched_at, 成功結果)}。只存 error=None 的成功快照，
# 供暫時性失敗（連線錯/429/5xx/壞 JSON）時回退，年齡上限 _LAST_GOOD_MAX_AGE。
_last_good: dict[str, tuple[float, dict]] = {}
_LAST_GOOD_MAX_AGE = 900.0  # 舊值可回退的最長秒數（15 分鐘；再舊的額度資訊已不可信）
...
def fetch_rate_limits(force: bool = False, cred_file: Path | None = None) -> dict:
    ...
    def _transient_failure() -> dict:
        # 暫時性失敗：15 分鐘內的 last-known-good 直接回舊值（標 stale），額度輪替
        # 等消費端才不會因一次 429 把帳號誤判成不可用而強制切走。
        lg = _last_good.get(key)
        if lg is not None and now - lg[0] <= _LAST_GOOD_MAX_AGE:
