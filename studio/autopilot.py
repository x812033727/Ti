...
def _write_status(
    ...
    *,
    ...
    state ∈ {"idle", "running", "quota_sleep", "budget_sleep", "rotate_restart"}；/api/autopilot 讀此檔
    回報「迴圈還活著、正在做什麼、睡到何時、各 provider 用量」。帳號輪替時 quota 另帶
    ``rotated_to``（切換目標 label）。寫入走 secure_write_root（與 backlog 同範式，
