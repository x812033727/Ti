...
FIELDS: tuple[Field, ...] = (
    ...
    *(
    ...
    Field(
        "TI_ALERT_EMAIL_TO",
        "Email 推播收件人（逗號分隔;與 SMTP host 皆填才啟用）",
        placeholder="ops@example.com",
        group="通知",
    ),
    Field(
        "TI_ALERT_SMTP_HOST",
        "SMTP host（空=關;Email 推播用）",
        placeholder="smtp.example.com",
        group="通知",
    ),
    Field(
        "TI_ALERT_SMTP_PORT",
        "SMTP port（465=SMTP_SSL;其他=STARTTLS;空=587）",
        placeholder="587",
        numeric=True,
        group="通知",
    ),
    ...
    Field(
        "TI_ALERT_FROM",
        "Email 寄件人",
        placeholder="Ti Studio <noreply@localhost>",
        group="通知",
    ),
    Field(
