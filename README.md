...
| `TI_TASK_ADMISSION` | 核心 autopilot 任務契約閘門：`off` 完整沿用舊流程；`shadow` 在正常流程只補契約、裁決與去敏 audit；`enforce` 才依五向結果分流。`.env` 是冷啟動預設；UI 設定先成為 shared desired，獨立 worker 在安全任務邊界才 ack 為 effective；從 `enforce` 降級時會先釋放既有 admission hold 再 ack | shadow |
...
- **任務准入**：正常取件時只有 `enforce` 會依裁決改變狀態或派工。它要求 v1 contract 的
  ...
  UI 儲存模式後只更新跨程序 control state 的 desired generation；autopilot 主線與調查旁路
  都空閒時才把它 ack 成 effective generation，執行中的任務不會途中換語意。從 `enforce`
  降到 `off`／`shadow` 時，既有 admission hold 會先一次性釋放回 legacy 派工，成功後才 ack；
  每次 admission commit／claim 都驗證 effective generation，切換端則用 backlog barrier
  排空已通過舊世代驗證的 commit。control state 只有 worker 能在安全邊界建立；遺失、損壞或
  不可寫時其他入口與 worker 都停止新 intake。worker 重啟不會用舊 `.env` 覆寫 shared desired。
  Autopilot 看板會同時顯示目前模式、要求模式與 pending generation，無須為這個欄位重啟 worker。
...
- **合併策略**：autopilot 開 PR 後一律走 `publisher._merge_flow`（等 CI→綠才 squash-merge），
  受保護分支由必過檢查自然攔下、不再有盲合的 `gh pr merge --admin`；CI 因基礎設施/帳務秒掛時，
  由 `TI_PUBLISH_BYPASS_INFRA_CI` 的繞過路徑處理。
- **解析規則**：`FORCE_PUSH` 只有 `0`、`false`、`False`、空值、未設定這五種會判為「關閉」，**其餘任何值一律視為開啟**。
