...
- select 欄須有「非法選項」案例。
- 須含「超長輸入不撐破版面」案例。
- 案例對齊實際欄位（39 個 select：4 基本＋2 Claude 模型＋8 角色模型＋2 Codex 控制＋2 Antigravity 控制＋8 每角色 provider＋13 進階組、
  8 combo、7 文字、4 秘密）。
...
def test_每個select欄都有非法選項案例(sec):
    ...
    checks = {
        ...
        "TI_CODEX_SANDBOX": ["TI_CODEX_SANDBOX", "Codex Sandbox"],
        "TI_CODEX_BYPASS_SANDBOX": ["TI_CODEX_BYPASS_SANDBOX", "Codex 完全停用"],
        "TI_ANTIGRAVITY_SANDBOX": ["TI_ANTIGRAVITY_SANDBOX", "Antigravity Sandbox"],
        "TI_ANTIGRAVITY_SKIP_PERMISSIONS": [
            "TI_ANTIGRAVITY_SKIP_PERMISSIONS",
            "Antigravity 自動核准",
        ],
        # 每角色 provider 覆寫（8 欄）：彙整於 2.27 一條案例，逐 env 列名
        "TI_PROVIDER_PM": ["TI_PROVIDER_PM"],
        "TI_PROVIDER_ENGINEER": ["TI_PROVIDER_ENGINEER"],
        "TI_PROVIDER_QA": ["TI_PROVIDER_QA"],
        "TI_PROVIDER_SENIOR": ["TI_PROVIDER_SENIOR"],
        "TI_PROVIDER_RESEARCHER": ["TI_PROVIDER_RESEARCHER"],
        "TI_PROVIDER_ARCHITECT": ["TI_PROVIDER_ARCHITECT"],
        "TI_PROVIDER_SECURITY": ["TI_PROVIDER_SECURITY"],
        "TI_PROVIDER_DEVOPS": ["TI_PROVIDER_DEVOPS"],
