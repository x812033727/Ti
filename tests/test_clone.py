...
注入／token／branch／sandbox 四項一律「攔截 run_command_exec」：用 async spy 取代
runner.run_command_exec，記錄 git_clone 實際組出的 argv list 並回傳可控的假 RunOutput。
所有驗證僅靠攔截到的 argv 與 tmp_path 副作用，全程不發起真實 clone。

註：git_clone 已遷移為參數式 exec（argv list），shell 不參與解析——url/branch 一律
當「單一 argv 元素」，metacharacter（`;`/`&&`/`$()`/backtick）天然不被解析，
不再需要 shlex.quote。本檔斷言因此改為檢查 argv 結構而非 shell 字串。
"""
...
def _forbid_real_subprocess(monkeypatch):
    ...
    版本。攔截 run_command_exec 的測試根本走不到這裡；萬一日後有人誤加「真跑 clone」
    的測試，會立刻在此炸開而非默默連網，釘死「所有驗證僅靠攔截 argv 與 tmp_path」。
    """
...
@dataclass
class CloneSpy:
    """攔截 run_command_exec 的 async spy：記錄呼叫（含 argv）並回傳可控的假 RunOutput。"""
    ...
    @property
    def last(self) -> dict:
        assert self.calls, "run_command_exec 未被呼叫"
        return self.calls[-1]
...
def clone_spy(monkeypatch):
    ...
    - 以 spy 取代 runner.run_command_exec（攔截路徑，git_clone 不會真的執行）
...
async def test_git_clone_no_token_passthrough(clone_spy, tmp_path):
    ...
    assert result.command == "git clone " + url
    assert "x-access-token:" not in result.command
    # 攔截到的實際 argv 不含 token 注入（公開倉庫）
    assert not any("x-access-token:" in p for p in clone_spy.last["argv"])
...
async def test_git_clone_url_injection_is_single_argv(clone_spy, tmp_path, payload_url):
    """惡意 url payload 必須是「單一 argv 元素」，走 exec 不經 shell，不被解析。

    主斷言：攔截到的 argv 中，整個 payload_url 原樣是「完整單一元素」；shell 不參與，
    `touch` / `pwned` 不可能被拆成獨立 argv 元素或被執行。
    """
    ...
    assert payload_url in argv
    # 主斷言 2：payload 未被拆成獨立的 touch / pwned 元素。
    ...
    # 輔助防呆（非主證據）：因為攔截了 run_command_exec，payload 根本沒被執行。
    assert not (tmp_path / "pwned").exists()


@pytest.mark.asyncio
async def test_git_clone_branch_injection_blocked(clone_spy, tmp_path):
    """惡意 branch payload（含 shell metachar）被 _BRANCH_RE 拒絕，不進入 argv。

    branch 注入由 _BRANCH_RE（^[\\w./-]+$）擋下：含 backtick/$/;/空格/換行的
    branch 不通過，git_clone 不附加 --branch，payload 完全不出現在 argv 中。
    ...
    for bad_branch in [
        ...
        argv = clone_spy.last["argv"]
        # branch 被拒：不附 --branch，且 payload 任何片段都不在 argv 裡。
        assert "--branch" not in argv
        assert not any("touch" in p or "pwned" in p for p in argv)
        # 輔助防呆（非主證據，攔截後 payload 未執行）。
        assert not (tmp_path / "pwned").exists()


# --- 任務 #3：_BRANCH_RE 參數注入過濾（PM #4）---------------------------
# 重點：exec 化只防「命令注入」，不防「參數注入」——以 -- 開頭的 branch 即使是單一
# argv 元素，仍會被 git 當成選項（如 --upload-pack=<cmd> 可導致 RCE）。git_clone
...
async def test_git_clone_branch_re_rejects(clone_spy, tmp_path, bad_branch):
    """負向：不合法／危險 branch 被 _BRANCH_RE 拒絕 → argv 不附加 --branch。
    ...
    argv = clone_spy.last["argv"]
    assert "--branch" not in argv
    # payload 不應以任何形式進入 argv（空字串例外，本就無內容可比對）。
    if bad_branch:
        assert bad_branch not in argv
...
async def test_git_clone_branch_re_accepts(clone_spy, tmp_path, good_branch):
    """正向：合法 branch 被接受 → argv 以相鄰的 `--branch <branch>` 附加。
    ...
    # --branch 與 branch 值必須相鄰且完全一致。
    idx = argv.index("--branch")
    assert argv[idx + 1] == good_branch
...
async def test_git_clone_forces_no_sandbox(clone_spy, tmp_path, token, branch):
    """git_clone 必須一律以 sandbox=False 呼叫 run_command_exec。

    clone 需要網路，而沙箱預設斷網（--unshare-net），若進沙箱會直接失敗。
    對應 runner.git_clone 的硬編 `sandbox=False`，任何參數組合都不得改變。
    """
    await runner.git_clone(
...
async def test_git_clone_never_spawns_real_subprocess(monkeypatch, tmp_path):
    """釘死「不碰網路」：故意「不」攔截 run_command_exec，讓 git_clone 走真實路徑。

    唯一對外（連網）的途徑就是 run_command_exec 內的 asyncio 子程序；本檔的 autouse
