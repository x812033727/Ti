...
切換，禁改 env。所有寫入端點與 WS 一律用 loopback client：門禁停用時
`require_admin` fail-safe 限本機，非 loopback 會先擋成 403。
"""
...
# 門禁停用時寫入端點 fail-safe 需 loopback 來源；TestClient 預設 host 非 IP（fail-closed）。
PORT = 12345
...
def _loopback_client(app) -> TestClient:
    """寫入端點一律用 loopback client：門禁停用時 require_admin fail-safe 限本機（403）。"""
...
def test_enabled_missing_current_password_returns_401(app, pw_env, monkeypatch):
    ...
    monkeypatch.setattr(config, "ACCESS_PASSWORD", "secret")
    client = _loopback_client(app)  # 沿用 loopback client（門禁啟用時來源已不影響判定）
...
def test_enabled_wrong_current_password_returns_403(app, pw_env, monkeypatch):
    ...
    detail 比對用以與 fail-safe loopback 的泛化 403 區分，防假綠燈。
    """
    monkeypatch.setattr(config, "ACCESS_PASSWORD", "secret")
    client = _loopback_client(app)
