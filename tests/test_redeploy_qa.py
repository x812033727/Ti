...
async def test_pull_main_uses_project_root_ff_only(monkeypatch):
    ...
    async def fake_run(cwd, cmd, timeout=None):
        captured["cwd"] = cwd
        captured["cmd"] = cmd
        return runner.RunOutput(cmd, 0, "Already up to date.", False)

    monkeypatch.setattr(runner, "run_command", fake_run)
    out = await redeploy.pull_main()
    assert out.ok
    assert captured["cwd"] == config.PROJECT_ROOT
    assert captured["cmd"] == "git pull --ff-only"  # fast-forward only，避免意外 merge commit
