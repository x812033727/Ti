...
設計重點：
  ...
  * 探針使用既有的 `/etc/hosts`，避開 `--bind ws`、`--tmpfs /tmp`、`~/.cache`
    等可寫或遮蔽區。沙箱內出現 EROFS 即確證 ro-bind 生效，測後再確認 host
    內容未被竄改。
"""
...
@pytest.fixture
def host_probe():
    """使用 ro-bind 根下的既有 host 檔，避免 repo 位於 /tmp 時被 tmpfs 遮蔽。

    刻意不放 `~/`、`~/.cache`、`/tmp`、cwd——這些路徑在沙箱內可能是 tmpfs、
    可寫 bind，或被 tmpfs 遮蔽，無法穩定驗到 ro-bind。
    """
    probe = Path("/etc/hosts")
    if not probe.exists():
        pytest.skip("/etc/hosts 不存在，無法驗證 ro-bind 既有檔案")
    return probe
...
def test_ro_bind_rejects_write_to_host_path(host_probe, tmp_path, monkeypatch):
    ...
    # ① 前置：該路徑在 host 可讀，後續用內容比對確認未被竄改。
    original = host_probe.read_text()
    assert original, "前置：host 探針應存在且可讀"
    ...
    # ③ 雙重保險：回 host 確認內容維持原樣（確實沒寫進去）。
    assert host_probe.read_text() == original, "host 探針內容不應被沙箱竄改"
