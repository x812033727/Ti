...
行的壓縮器；同一組 parser 等價檢查在四個函式都必須 raise AssertionError，
避免只測到 no-op 假綠。
"""
...
def _parse_engineer_mentions(text: str) -> object:
    return parse_mentions("工程師", text, ["工程師", "架構師", "測試員"])
...
CASES = (
    ParserCase(
        ...
        text=_tail_marker_text(PREV_SEGMENT_MAX_CHARS, "驗證: FAIL"),
        short_text="QA 結論\n驗證: FAIL",
    ...
    ParserCase(
        ...
        text=_tail_marker_text(SELF_SEGMENT_MAX_CHARS, "決議: 退回"),
        short_text="審查結論\n決議: 退回",
    ...
    ParserCase(
        ...
        text=_tail_marker_text(
            PREV_SEGMENT_MAX_CHARS,
            "核心改動: [P0/bug] 修正討論摘要路由",
        ),
        short_text="核心改動: [P0/bug] 修正討論摘要路由",
...
def test_clip_keeps_tail_marker_parser_result(case: ParserCase) -> None:
    assert len(case.text) > PREV_SEGMENT_MAX_CHARS
...
def test_black_samples_catch_marker_eating_compressor(case: ParserCase) -> None:
    """破壞版壓縮器吃掉尾段 marker 時，四函式等價檢查都會攔下。"""
