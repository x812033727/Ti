...
def test_no_bare_python_studio_server():
    ...
    bad = [ln for ln in CODE
           if re.search(r"(?<![/\\])\bpython3? -m studio", ln)]
...
def test_no_bare_pip_install():
    ...
    bad = [ln for ln in CODE
           if re.search(r"(?<!-m )(?<![/\\.])\bpip install", ln)]
...
def test_no_bare_uvicorn_command():
    ...
    bad = [ln for ln in CODE
           if re.search(r"(?<!-m )(?<![./\\])\buvicorn studio", ln)]
...
def test_all_server_launches_use_venv_path():
    ...
    launches = [ln for ln in CODE if "studio.server" in ln and "-m" in ln
...
def test_install_section_points_to_env_setup():
    ...
    m = re.search(r"^##\s+安裝\s*$(.*?)(?=^##\s|\Z)",
                  README, re.MULTILINE | re.DOTALL)
    assert m, "找不到『安裝』段"
    sec = m.group(1)
    # 安裝段不得殘留裸 `pip install -e .` fallback
    assert not re.search(r"(?<![/\\.])\bpip install -e \.", sec), \
        "安裝段殘留裸 pip install -e ."
    # 應引用前置段（連結或 .venv 寫法）
    assert "執行環境前置" in sec or ".venv/bin/python3" in sec, \
        "安裝段未對齊/引用前置段"
